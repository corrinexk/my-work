...
def train_n_epochs(n_epochs, model, label_type, train_loader, valid_loader, criterion, optimizer, savefile, early_stop_epoch):
    ...
    for epoch_i in range(1, n_epochs+1):
        ...
        ##data是图片，ret5, ret20是标记
        for i, (data, ret5, ret20,date_first,date_last) in enumerate(train_loader):
            ...
            ##比较输出和标记，不断比较output, target的loss进行训练
        ...
        for i, (data, ret5, ret20,date_first,date_last) in enumerate(valid_loader):
            assert label_type in ['RET5', 'RET20'], f"Wrong Label Type: {label_type}"
            if label_type == 'RET5':
                target = ret5
            else:
        ...
        # if validation loss gets smaller, save the model,保存验证误差最小的，作为模型
        ...
        ##加大轮数，继上一个保存的模型以来，如果超过early_stop_epoch轮valid的loss还没有下降就退出
        ##防止轮数太大过拟合
...
def plot_loss_and_acc(loss_and_acc_dict):
    ...
    #axes[0].set_xticks(range(0, maxEpoch + 1, maxEpoch//10))改，步数不能为0
    axes[0].set_xticks(range(0, maxEpoch + 1, 1))
    ...
    #axes[1].set_xticks(range(0, maxEpoch + 1, maxEpoch//10))改
    axes[1].set_xticks(range(0, maxEpoch + 1, 1))
